...
class Teacher2(_nn.Module):
    ...
    '''
    def __init__(self, 
        img_size: _typing.Union[_np.ndarray, _typing.List[int]], 
        num_classes: int,
        noise_size: int = 128, 
        ):
        '''
        ...
        self.model = _nn.Sequential(
            ...
            _nn.ConvTranspose2d(64, img_size[0], 3, 1, padding = 1),
            _nn.Tanh()
    ...
    def forward(self, x, noise):
        x = _torch.cat([x, noise], dim=1)      ## Concatenating Noise and Label Vectors :     (? , noise_vec + num_classes)        
        return self.model(x) 
...
class Teacher1(_nn.Module):
    ...
    def __init__(self, 
        ...
        self.model=_nn.Sequential(
            ...
            _nn.LeakyReLU(0.1),
            _nn.Unflatten(1, ( fc2_filters, img_size[1] , img_size[2])),
            ...
            _weight_norm(_nn.Conv2d(conv1_filters, img_size[0], 3, 1, padding= "same")),
            _nn.Tanh()
        )
...
class Learner(_nn.Module):
    ...
    def __init__(self, 
        img_size: list, 
        num_classes: int, 
        cnn_filters: _typing.Union[_typing.List[int],_np.ndarray , None] = None, 
        linear_filters: _typing.Union[_typing.List[int],_np.ndarray , None] =None):
        ''' Learner model.
        Parameters:
        ...
        cnn_filters = [img_size[0]]+cnn_filters
        ...
        initial_filters= cnn_filters[-1] * img_size[1]//(2**cnn_layers) * img_size[2]//(2**cnn_layers) 
